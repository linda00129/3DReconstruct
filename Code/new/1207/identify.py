...
class IdentifyClass:
    ...
    def ransacLine(self, Id, sigma=50):
        best_a, best_b = 0, 0
        maxTot = 0
        ret = []
        iters = 10000
    ...
    def identify(self):
        ...
        for i in range(self.n1):
            self.planePointsId[i] = self.ransacPlane(Id1, sigma=self.sigma)
            ...
            for j in range(self.n2):
                ...
                self.linePointsId[i][j] = self.ransacLine(Id2, sigma=50)
        ...
        while True:
            ...
            for i in range(self.n1):
                ...
                while len(tmp) != self.n2 * self.n3 and cnt < 100:
                    tmp = self.ransacPlane(Id1, self.sigma)
                    cnt += 1
                if len(tmp) != self.n2 * self.n3:
                    print('Error: len(tmp) = ', len(tmp))
                    continue
                ...
                # TODO: Add another check condition
                flag1 = flag2 = 0
                for j in Id1:
                    _z = self.plane[i][0] * self.A[j][0] + self.plane[i][1] * self.A[j][1] + self.plane[i][2];
                    if (_z < self.A[j][2]):
                        if (flag2 != 0):
                            bad = 1
                            break
                ...
                if (abs(self.plane[i][0]) > 2 or abs(self.plane[i][1]) > 2):
                    bad = 1
                if (bad == 1):
                    break
                ...
                for j in range(self.N):
                    ...
                    minX = min(minX, self.A[j][0])
                    maxX = max(maxX, self.A[j][0])
                    minY = min(minY, self.A[j][1])
                    maxY = max(maxY, self.A[j][1])
    ...
    def check(self, scaling=False, lx=0, ly=0, lz=0):
        ...
        if (scaling == True):
            scaleZ = lz / lenZ[0][0][0]
            scaleY = ly / lenY[0][0][0]
            scaleX = lx / lenX[0][0][0]
            print('前后缩放比例 = ', scaleZ)
            print('竖直缩放比例 = ', scaleY)
            print('水平缩放比例 = ', scaleX)
            for i in range(self.n1):
        ...
        print('sd = ', np.std(lenZ))

        print()
        print('竖直')
        print(lenY)
        print('mean = ', np.mean(lenY))
        print('sd = ', np.std(lenY))

        print()
        print('水平')
        print(lenX)
        print('mean = ', np.mean(lenX))
        print('sd = ', np.std(lenX))
